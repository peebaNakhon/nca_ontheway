...
class InHouseDown extends StatefulWidget {
  ...
  const InHouseDown({super.key, required this.par_qtimedt, required this.action, required this.pointid, required this.pageName, required this.par_busdata});

  @override
  _InHouseDownState createState() => _InHouseDownState();
}

class _InHouseDownState extends State<InHouseDown> {
                    ...
                    child: ListTile(
                      title: Column(
                        ...
                        children: [
                          ...
                          Text(
                            "จาก : ${itemList[index]['frompoint']}",
                            style: const TextStyle(fontWeight: FontWeight.normal),
                          ),
                          (itemList[index]['statusup'] == 0)
                              ? const Text(
                                  "สถานะต้นทาง : ยังไม่รับขึ้นรถ",
                                  style: TextStyle(color: Colors.red),
                                )
                              : const Text(
                                  "สถานะต้นทาง : รับขึ้นรถแล้ว",
                                  style: TextStyle(color: Colors.green),
                                ),
                          (itemList[index]['statusdown'] == 0)
                              ? const Text(
                                  "สถานะปลายทาง : ยังไม่นำลงรถ",
                                  style: TextStyle(color: Colors.red),
                                )
                              : const Text(
                                  "สถานะปลายทาง : นำลงจากรถแล้ว",
                                  style: TextStyle(color: Colors.green),
                                )
          ...
          actions: <Widget>[
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                Navigator.of(context).pop();
                await ApiCalling()
                    .saveSend(RefNo, widget.par_busdata["queue_busnumber"], widget.par_busdata["queue_busline"], widget.par_busdata["queue_buslinetype"],
  ...
  Future<void> ncaParcelAction(String refNo) async {
    ...
    try {
      ...
      //       duration: const Duration(seconds: 3));
      // }
      await EasyLoading.showSuccess("บันทึกสำเร็จ", duration: const Duration(seconds: 3));
