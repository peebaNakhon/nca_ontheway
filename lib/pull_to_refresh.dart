...
class DragGesturePullToRefresh extends VerticalDragGestureRecognizer {
  ...
  FixedScrollMetrics _getMetrics(double minScrollExtent, double maxScrollExtent, double pixels, double viewportDimension, AxisDirection axisDirection) {
    return FixedScrollMetrics(
        minScrollExtent: minScrollExtent, maxScrollExtent: maxScrollExtent, pixels: pixels, viewportDimension: viewportDimension, axisDirection: axisDirection, devicePixelRatio: 1);
  }
  ...
  DragGesturePullToRefresh([this.msWaitToRestart = exceedsLoadingTime, this.dragStartYDiff = 0]) {
    onStart = (DragStartDetails dragDetails) async {
      ...
      if (!isLoading ||
        ...
        if (scrollPos.dy <= dragStartYDiff) {
          dragStarted = true;
          dragDistance = 0;
          ScrollStartNotification(metrics: _getMetrics(0, dragHeightEnd, 0, dragHeightEnd, AxisDirection.down), dragDetails: dragDetails, context: _context).dispatch(_context);
        }
      }
    };
    onUpdate = (DragUpdateDetails dragDetails) {
      if (dragStarted) {
        ...
        ScrollUpdateNotification(
                metrics: _getMetrics(dy > 0 ? 0 : dragDistance, dragHeightEnd, dy > 0 ? (-1) * dy : dragDistance, dragHeightEnd, dragDistance < 0 ? AxisDirection.up : AxisDirection.down),
                context: _context,
                scrollDelta: (-1) * dy)
            .dispatch(_context);
        if (dragDistance < 0) {
    ...
    onEnd = (DragEndDetails dragDetails) {
      if (dragStarted) {
        ScrollEndNotification(metrics: _getMetrics(0, dragHeightEnd, dragDistance, dragHeightEnd, AxisDirection.down), context: _context).dispatch(_context);
        _clearDrag();
      }
    };
    onCancel = () {
      if (dragStarted) {
        ScrollUpdateNotification(metrics: _getMetrics(0, dragHeightEnd, 1, dragHeightEnd, AxisDirection.up), context: _context, scrollDelta: 0).dispatch(_context);
        _clearDrag();
      }
    };
  }
